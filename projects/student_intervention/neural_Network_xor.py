...
Network = [
    ...
    [ Perceptron(np.array([1,0]),0.5),Perceptron(np.array([0.5,0.5]),0.75),Perceptron(np.array([0,1]),0.5) ], 
    # output node, declare output layer perceptron here
    [ Perceptron(np.array([1,-2, 1]), 0.9)]
]
...
def EvalNetwork(inputValues, Network):
    ...
    for layers in Network:
        new_inputs = []
        for precep in layers:
            new_inputs.append(precep.activate(inputValues))
        # print new_inputs
        inputValues = np.asarray(new_inputs)
